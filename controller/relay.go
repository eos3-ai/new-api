...
func shouldSkipAutoDisable(tag string) bool {
...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError) {
	...
	// 检查是否应该跳过自动禁用（带有 no_auto_test 标签的渠道不会被自动禁用）
	skipAutoDisable := shouldSkipAutoDisable(channelError.Tag)
	common.SysLog(fmt.Sprintf("processChannelError: channel #%d, tag='%s', skipAutoDisable=%v, AutoBan=%v", channelError.ChannelId, channelError.Tag, skipAutoDisable, channelError.AutoBan))
	if service.ShouldDisableChannel(channelError.ChannelType, err) && channelError.AutoBan && !skipAutoDisable {
		gopool.Go(func() {
